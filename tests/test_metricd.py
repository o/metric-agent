...
class TestMetricD(unittest.TestCase):
    def test_merge_dicts(self):
        collector = SystemCollector(10)
        self.assertDictEqual({'foo': 'bar', 'baz': 'bogus'}, collector.merge_dicts({'foo': 'bar'}, {'baz': 'bogus'}))

    def test_get_derived(self):
        utils = SystemCollector(10)
        self.assertEqual(0, utils.get_derived('foo', 10))
        self.assertEqual(1, utils.get_derived('foo', 20))
        self.assertEqual(10, utils.get_derived('foo', 120))
